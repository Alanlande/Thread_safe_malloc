...
int main(int argc, char *argv[])
{
  ...
  //printf("End counting and start checking\n"); 
  ...
  if (fail == 0) {
    ...
    printf("Region 1 bounds: start=%llx, end=%llx, size=%dB, idx=%d\n", (long long)start, (long long)end, (int)malloc_items[i].bytes, i);
    printf("Region 2 bounds: start=%llx, end=%llx, size=%dB, idx=%d\n", (long long)tgt_start, (long long)tgt_end, (int)malloc_items[j].bytes, j);
    printf("Test failed\n");
  } //else
